...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (data && data.type === "page" && p === "home") {
      const fetchAndInject = async () => {
        try {
          ...
          if (casesContainer)
            casesContainer.innerHTML = generateHtml(casesRes.data, "case");

          const newsContainer = document.getElementById("dynamic-home-news");
          if (newsContainer)
            newsContainer.innerHTML = generateHtml(newsRes.data, "news");

          const blogsContainer = document.getElementById("dynamic-home-blogs");
          if (blogsContainer)
            blogsContainer.innerHTML = generateHtml(blogsRes.data, "blog");
        } catch (e) {
    ...
    setTimeout(() => {
      if (window.Swiper) {
        document
          .querySelectorAll(".swiper:not(.swiper-initialized)")
          .forEach((el) => {
            const settingsStr = el
              .closest(".elementor-widget")
              ?.getAttribute("data-settings");
            const nextEl = el.parentNode.querySelector(
              ".elementor-swiper-button-next",
            );
            const prevEl = el.parentNode.querySelector(
            ...
            try {
              if (settingsStr) {
                ...
                if (settings.autoplay_speed)
                  options.autoplay.delay = settings.autoplay_speed;
                if (settings.slides_per_view)
                  options.slidesPerView =
                    parseInt(settings.slides_per_view) || 1;
                if (settings.space_between)
                  options.spaceBetween = parseInt(settings.space_between) || 0;
                if (settings.slides_per_view_tablet) {
                  options.breakpoints[768] = {
                    slidesPerView:
                      parseInt(settings.slides_per_view_tablet) || 1,
                  };
                }
                if (settings.slides_per_view_mobile) {
                  options.breakpoints[480] = {
                    slidesPerView:
                      parseInt(settings.slides_per_view_mobile) || 1,
                  };
                }
              }
            } catch (e) {}
            ...
            try {
              ...
              if (settingsStr && videoContainer && !videoContainer.innerHTML) {
                ...
                if (settings.video_type === "youtube" && settings.youtube_url) {
                  const match =
                    settings.youtube_url.match(/[?&]v=([^&]+)/) ||
                    settings.youtube_url.match(/youtu\.be\/([^?]+)/);
                  if (match && match[1]) {
                  ...
                  const match = settings.vimeo_url.match(
                    /vimeo\.com\/(?:video\/)?([0-9]+)/,
                  );
                  if (match && match[1]) {
                    let params = "?";
                ...
                if (url) {
                  videoContainer.innerHTML =
                    '<iframe class="elementor-video-iframe" allowfullscreen="1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" src="' +
                    url +
                    '" width="100%" height="100%" frameborder="0" style="position:absolute;top:0;left:0;width:100%;height:100%;"></iframe>';
                  ...
                  if (settings.aspect_ratio) {
                    ...
                    if (ratio === "169")
                      ...
                      videoContainer.style.paddingBottom = "42.85%";
                    else if (ratio === "43")
                      videoContainer.style.paddingBottom = "75%";
                    else if (ratio === "32")
                      videoContainer.style.paddingBottom = "66.66%";
              ...
              console.error("Error initializing video widget", e);
            }
          });
        ...
        document.querySelectorAll("[data-settings]").forEach((el) => {
          try {
            ...
            if (
              settings.background_background === "slideshow" &&
              settings.background_slideshow_gallery
            ) {
              ...
              const transitionStr =
                settings.background_slideshow_slide_transition || "fade";
              const speed =
                settings.background_slideshow_transition_duration || 500;
              const delay =
                settings.background_slideshow_slide_duration || 5000;

              // Create container
              const bgWrapper = document.createElement("div");
              bgWrapper.className =
                "elementor-background-slideshow swiper swiper-initialized";
